...
const printarApartementoEspecifico = (
  opRegiao: number,
  opMetragem: number
): void => {
  console.log(
    `${Regiao[opRegiao]} ${Metragem[opMetragem]} = ${precos[opRegiao][opMetragem]}`
  );
};

const mediaPrecosMetragem = (opMetragem: number): void => {
  let temp: number = 0;
  for (let i = 0; i < 5; i++) {
    temp += precos[i][opMetragem];
  }
  const media = temp / 5;

  console.log(`Meida dos preços de ${Metragem[opMetragem]}: ${media}R$`);
};
...
console.log(
  "[2] Ver o preço do apartamento mais barato de acordo com a metragem"
);
...
switch (op) {
  ...
  case 2:
    ...
    printarMenorValorMetragem(opMetragem - 1);
  ...
  case 3:
    ...
    opMetragem = parseInt(prompt("Opção: "));
    console.log();
    printarApartementoEspecifico(opRegiao - 1, opMetragem - 1);
    break;
  case 4:
    console.log("############################");
    console.log("Escolha uma metragem");
    ...
    opMetragem = parseInt(prompt("Opção: "));
    console.log();
    mediaPrecosMetragem(opMetragem - 1);
